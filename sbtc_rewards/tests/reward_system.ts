...
import { generateAndSaveKeypair, loadKeypair, requestAirdrop } from "./utils"; // Adjust the path as needed
...
describe("RewardSystem", () => {
  ...
  it('should request SOL from the faucet for testKeypair2', async () => {
    await requestAirdrop(testKeypair2, 2 * anchor.web3.LAMPORTS_PER_SOL);
  });
  ...
  it('should get locks information for testKeypair2', async () => {
    await getLocksInformation(program, testKeypair2);
  });
  ...
  it('should lock tokens for testKeypair2', async () => {
    await lockTokens(program, testKeypair2, new BN(2100000000));
  });
  ...
  it('should unlock tokens for testKeypair2', async () => {
    await unlockTokens(program, testKeypair2, new BN(1), new BN(10000000));
  });
});
